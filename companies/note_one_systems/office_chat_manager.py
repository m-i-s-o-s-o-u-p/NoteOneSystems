    ...
    def _simulate_expert_response(self, emp_id: str, query: str, emp: Dict[str, Any]) -> str:
        ...
        if any(k in q for k in ["雇用", "採用", "増員", "配属", "recruit", "hire"]):
            return (
                ...
                "### 🤝 新規AI社員を雇用するための2つの方法\n"
                "1. **人事課（HR）画面の「増員提案のワンクリック採用」:**\n"
                "   - 現在、チーフライターの森川 拓真（業務負荷100%）に対して増員提案が起案されています。「🤝 人事課」画面を開いていただき、**【この増員提案を承認して即時雇用する (0円)】**ボタンを押していただくことで、即座にアシスタントライター（桐生 蓮）が配属されます。\n"
                "2. **人事課（HR）画面の「即時採用デスク / 自由指定採用フォーム」:**\n"
                "   - 「🤝 人事課」画面の採用デスクから、SEOアナリスト、アイキャッチデザイナー等の推薦スペシャリストを選んでワンクリック採用するか、お好きな名前・役職で完全オリジナルのAI社員を即時雇用いただけます。\n"
                "3. **追加費用:**\n"
