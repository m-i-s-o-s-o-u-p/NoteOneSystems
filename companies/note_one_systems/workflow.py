...
class NoteOneWorkflow:
    ...
    def run_creation_pipeline(self, topic: str, target_audience: str = "", price_preference: str = "自動提案") -> Generator[Dict[str, Any], None, Dict[str, Any]]:
        ...
        # Step 1: リサーチ (風間)
        yield {"step": 1, "employee": "kazama", "status": "thinking", "message": f"風間 涼 がテーマ「{topic}」の市場ニーズ・競合・読者ペルソナを分析中..."}
        ...
        meeting_logs.append({"employee": "kazama", "name": "風間 涼", "role": "市場リサーチ担当", "icon": "🔍", "content": res_kazama})
        yield {"step": 1, "employee": "kazama", "status": "done", "log": meeting_logs[-1]}
        time.sleep(0.3)

        # Step 2: 財務 (白石)
        yield {"step": 2, "employee": "shiraishi", "status": "thinking", "message": "白石 葵 が最適価格と売上シミュレーションを算出中..."}
        ...
        meeting_logs.append({"employee": "shiraishi", "name": "白石 葵", "role": "財務アナリスト", "icon": "📊", "content": res_shiraishi})
        yield {"step": 2, "employee": "shiraishi", "status": "done", "log": meeting_logs[-1]}
        time.sleep(0.3)

        # Step 3: CEO承認 (一条)
        yield {"step": 3, "employee": "ichijo", "status": "thinking", "message": "一条 蓮 が企画を最終レビューし、編集長へ指示を下達中..."}
        sys_ichijo = self._get_employee_prompt("01_ceo_ichijo")
        prompt_ichijo = f"テーマ: {topic}\nリサーチ:\n{res_kazama}\n財務分析:\n{res_shiraishi}\n結城編集長と森川ライターへ執筆方針を伝えてください。"
        ...
        if not res_ichijo:
            res_ichijo = (
                ...
                f"結城編集長、価格500円に見合う圧倒的な構成案を作成してください。\n"
                f"森川ライター、読者が今日からすぐ使える実践テンプレの執筆をお願いします！"
            )
        ...
        # Step 4: 編集長構成 (結城)
        yield {"step": 4, "employee": "yuki", "status": "thinking", "message": "結城 紬 が記事の構成・目次・無料/有料ラインを設計中..."}
        sys_yuki = self._get_employee_prompt("05_editor_yuki")
        prompt_yuki = f"テーマ: {topic}\nリサーチ:\n{res_kazama}\nCEO指示:\n{res_ichijo}\nライター森川に向けた詳細な構成指示書を作成してください。"
        ...
        meeting_logs.append({"employee": "yuki", "name": "結城 紬", "role": "統括編集長", "icon": "📑", "content": res_yuki})
        yield {"step": 4, "employee": "yuki", "status": "done", "log": meeting_logs[-1]}
        time.sleep(0.3)

        # Step 5: ライター執筆 (森川)
        yield {"step": 5, "employee": "morikawa", "status": "thinking", "message": "森川 拓真 が本文・実践ノウハウ・テンプレートを本格執筆中..."}
...
"""
        ...
        meeting_logs.append({"employee": "morikawa", "name": "森川 拓真", "role": "チーフライター", "icon": "✍️", "content": "原稿の執筆が完了しました！編集長の構成指示に忠実に、即効性の高いテンプレを盛り込んでいます。法務顧問とQA担当の審査をお願いします。"})
        yield {"step": 5, "employee": "morikawa", "status": "done", "log": meeting_logs[-1]}
        time.sleep(0.3)

        # Step 6: 法務審査 (橘)
        yield {"step": 6, "employee": "tachibana", "status": "thinking", "message": "橘 律 が会社法・著作権法・note規約適合性を審査中..."}
        ...
        meeting_logs.append({"employee": "tachibana", "name": "橘 律", "role": "法務・コンプライアンス顧問", "icon": "⚖️", "content": res_tachibana})
        yield {"step": 6, "employee": "tachibana", "status": "done", "log": meeting_logs[-1]}
        time.sleep(0.3)

        # Step 7: QA採点 (神崎)
        yield {"step": 7, "employee": "kanzaki", "status": "thinking", "message": "神崎 玲奈 がファクトチェック・読者満足度を100点満点で審査中..."}
        ...
        meeting_logs.append({"employee": "kanzaki", "name": "神崎 玲奈", "role": "品質管理責任者 (QA)", "icon": "🛡️", "content": res_kanzaki})
        yield {"step": 7, "employee": "kanzaki", "status": "done", "log": meeting_logs[-1]}
        time.sleep(0.3)

        # Step 8: 5大SNSプロモーション (佐々木)
        yield {"step": 8, "employee": "sasaki", "status": "thinking", "message": "佐々木 翼 が5大SNS（X, IG, Threads, Bluesky, Mastodon）向け告知文を作成中..."}
...
"""
        ...
        meeting_logs.append({"employee": "sasaki", "name": "佐々木 翼", "role": "マルチSNSマーケター", "icon": "📢", "content": "5大SNS（X, Instagram, Threads, Bluesky, Mastodon）向けのプロモーションパッケージが完成しました！"})
        ...
        meeting_logs.append({
            ...
            "role": "人事責任者",
            "icon": "🤝",
            "content": "全工程の業務負荷を記録しました。ライター森川さんの執筆負荷が高まっていますが、各社員が就業規則を遵守して円滑に連携しています。"
        })
