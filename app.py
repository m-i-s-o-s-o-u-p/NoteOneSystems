...
# ==========================================
    ...
    if not pr_approved_articles:
        ...
        st.markdown(f"#### 📢 {'5大SNSプロモーション原稿（全媒体一括表示・タブ完全廃止）' if lang=='ja' else '5 Major SNS Syndication Drafts (Direct View)'}")
        st.markdown(f"<div style='color: #94A3B8; font-size: 0.88rem; margin-bottom: 14px;'>{'広報課（佐々木 翼）が作成した各SNS向け告知文です。タブによる切り替えを完全廃止し、すべての媒体の投稿文をスクロールして即時確認・コピーできます。' if lang=='ja' else 'Direct syndication texts across all 5 major platforms generated by PR Hub.'}</div>", unsafe_allow_html=True)

        with st.expander("📋 5大SNSプロモーションパッケージ 全文（一括コピー用）", expanded=False):
            st.caption("全5媒体分のテキストが一括で格納されています。右上のコピーアイコンでそのまま取得できます。")
            st.text_area("全文一括テキスト", value=pr_marketing_raw, height=220, key=f"copy_all_{target_pr_art['id']}")
        ...
        with col_pr1:
            ...
            <div style="display: flex; align-items: center; gap: 8px; margin-bottom: 4px;">
                ...
                <strong style="color: #38BDF8; font-size: 0.95rem;">X (Twitter) 向け短文告知ポスト</strong>
                <span style="color: #64748B; font-size: 0.75rem;">（拡散重視・ハッシュタグ付）</span>
            </div>
            """, unsafe_allow_html=True)
            st.text_area("X投稿文", value=x_text, height=140, key=f"copy_x_{target_pr_art['id']}", label_visibility="collapsed")

            st.markdown("""
            <div style="display: flex; align-items: center; gap: 8px; margin-top: 14px; margin-bottom: 4px;">
                <span style="font-size: 1.2rem;">🧵</span>
                <strong style="color: #E2E8F0; font-size: 0.95rem;">Threads 思考ログ・ストーリー型ポスト</strong>
                <span style="color: #64748B; font-size: 0.75rem;">（共感・体験談重視）</span>
            </div>
            """, unsafe_allow_html=True)
            st.text_area("Threads投稿文", value=th_text, height=140, key=f"copy_th_{target_pr_art['id']}", label_visibility="collapsed")

            st.markdown("""
            <div style="display: flex; align-items: center; gap: 8px; margin-top: 14px; margin-bottom: 4px;">
                <span style="font-size: 1.2rem;">🐘</span>
                <strong style="color: #A78BFA; font-size: 0.95rem;">Mastodon トピック告知ポスト</strong>
                <span style="color: #64748B; font-size: 0.75rem;">（技術・専門コミュニティ向け）</span>
            </div>
            """, unsafe_allow_html=True)
            st.text_area("Mastodon投稿文", value=mast_text, height=140, key=f"copy_mast_{target_pr_art['id']}", label_visibility="collapsed")

        with col_pr2:
            st.markdown("""
            <div style="display: flex; align-items: center; gap: 8px; margin-bottom: 4px;">
                <span style="font-size: 1.2rem;">📸</span>
                <strong style="color: #F472B6; font-size: 0.95rem;">Instagram カルーセルスライド構成＆キャプション</strong>
                <span style="color: #64748B; font-size: 0.75rem;">（スライド展開・視覚重視）</span>
            </div>
            """, unsafe_allow_html=True)
            st.text_area("Instagram投稿文", value=ig_text, height=140, key=f"copy_ig_{target_pr_art['id']}", label_visibility="collapsed")

            st.markdown("""
            <div style="display: flex; align-items: center; gap: 8px; margin-top: 14px; margin-bottom: 4px;">
                <span style="font-size: 1.2rem;">🦋</span>
                <strong style="color: #60A5FA; font-size: 0.95rem;">Bluesky 要約告知ポスト</strong>
                <span style="color: #64748B; font-size: 0.75rem;">（オープンプロトコル・速報型）</span>
            </div>
            """, unsafe_allow_html=True)
            st.text_area("Bluesky投稿文", value=bs_text, height=140, key=f"copy_bs_{target_pr_art['id']}", label_visibility="collapsed")
...
# ==========================================
    ...
    # 1. おすすめ即戦力スペシャリスト一覧
    st.markdown(f"##### 🎯 {'人事課おすすめの即戦力スペシャリスト (未採用' + str(preset_count) + '名 / 費用0円・即時配属)' if preset_count > 0 else '人事課おすすめの即戦力スペシャリスト (全員配属済み)'}")
    if not presets:
        st.markdown(f"""
        <div style='background: #0F172A; border: 1px solid #10B981; border-radius: 8px; padding: 16px 18px; margin-bottom: 16px;'>
            <div style='font-size: 1.15rem; margin-bottom: 6px; color: #10B981;'>🎉 <strong>おすすめ即戦力スペシャリストは全員採用・配属済みです！</strong></div>
            <div style='color: #E2E8F0; font-size: 0.88rem; line-height: 1.6;'>
                人事課が推薦する即戦力AI社員はすべて雇用され、各部署および2Dオフィスフロアでフル稼働しています。<br>
                別分野の専門職をさらに増員したい場合は、下記の「🗣️ 綾瀬七海への採用オーダー」または「✍️ 自由指定 採用フォーム」より自由な役職名・スキルで何名でも即時採用（永久0円）可能です。
            </div>
        </div>
        """, unsafe_allow_html=True)
    else:
    ...
    # 2. 🗣️ 綾瀬七海への採用オーダーデスク（AIスカウト）
    st.markdown("<div style='height: 10px;'></div>", unsafe_allow_html=True)
    st.markdown(f"##### {'🗣️ 綾瀬七海（人事責任者）への採用オーダーデスク（AIスカウト・完全無料）' if lang=='ja' else '🗣️ AI Scout Order Desk'}")
    ...
    # 3. ✍️ オーナー自由指定 採用フォーム
    st.markdown("<div style='height: 10px;'></div>", unsafe_allow_html=True)
    with st.expander("✍️ オーナー自由指定 採用フォーム（完全カスタムAI社員・役職名やスキルを自由に設計・費用0円）" if lang=='ja' else "✍️ Custom Recruitment Form (Zero Cost)", expanded=False):
